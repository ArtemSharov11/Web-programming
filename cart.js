...
async function updateQty(id, newQty) {
    ...
    showToast("Количество обновлено.", "info");
    fetchCart();
}

async function removeItem(id) {
    const confirmed = await showConfirm("Удалить услугу из корзины?", "Корзина");
    if (!confirmed) return;
    await fetch(`${API_URL}/cart/${id}`, { method: 'DELETE' });
    showToast("Услуга удалена из корзины.", "success");
    fetchCart();
}
...
document.getElementById('checkout-btn').onclick = async () => {
    ...
    if (!user) {
        showToast("Для оформления заказа необходимо авторизоваться.", "error");
    ...
    try {
        ...
        showToast("Покупка успешно оформлена. Корзина очищена.", "success");
        fetchCart();
        
    } catch (error) {
        console.error("Ошибка при оформлении заказа:", error);
        showToast("Произошла ошибка при сохранении заказа.", "error");
    }
};
