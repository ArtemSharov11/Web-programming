...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (!user || user.role !== "admin") {
        alert("Доступ запрещён. Эта страница доступна только администратору.");
...
async function saveService(event) {
    ...
    resetForm();
    await loadServices();
    alert(id ? "Услуга обновлена." : "Услуга добавлена.");
}
...
async function editService(id) {
    ...
    if (!response.ok) return alert("Услуга не найдена.");
...
async function deleteService(id) {
    ...
    if (!response.ok) return alert("Не удалось удалить услугу.");
    await loadServices();
}
...
async function deleteReview(id) {
    ...
    if (!response.ok) return alert("Не удалось удалить отзыв.");
    await loadReviews();
}
